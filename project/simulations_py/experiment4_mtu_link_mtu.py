#!/usr/bin/env python3
"""
Experiment 4 — Variant: Link MTU Variation
===========================================
Modification: Fix the application payload at 1000 bytes (1028 bytes on the
wire including IP+UDP headers) and sweep the link MTU from 512 B up to 1500 B.

Justification: The base Experiment 4 fixes the MTU and varies the payload.
This variant flips the perspective — it fixes the payload and varies the MTU,
modeling the real-world scenario where an operator tunes the link MTU (e.g.,
to accommodate VXLAN overhead, PPPoE headers, or different hardware limits).
The crossover occurs at MTU = 1028 B: below it every packet is dropped;
at or above it packets are delivered without loss.  This shows that MTU
provisioning is a binary on/off knob for a given traffic class.
"""
import csv
from ns import ns
...
# Fixed application payload — chosen so the on-wire frame is 1028 B,
# which sits between standard MTU values to make the crossover visible.
PAYLOAD_SIZE   = 1000                         # bytes
FRAME_SIZE     = PAYLOAD_SIZE + IP_UDP_HEADER_BYTES   # = 1028 B on the wire

# Sweep link MTU values: below, at, and above the 1028-byte frame boundary.
...
def main():
    ...
    print(f"\nExperiment 4 (link MTU variation) results written to: {output_csv}")
