#!/usr/bin/env python3
"""Experiment 4: wired-vs-wireless packet-size comparison.

Topology baseline is the same as Experiment 3:
Sender1 --+
Sender2 --+-- Router1 --[bottleneck p2p]-- Router2 --[last hop]-- Receiver
Sender3 --+

For direct comparison, this script runs two last-hop options for each packet size:
- wireless: Router2 -> Receiver via 802.11g WiFi (AP/STA)
- wired:    Router2 -> Receiver via point-to-point 10 Mbps, 2 ms

All other parameters are identical across link types:
- 3 senders, each 4 Mbps offered load
- bottleneck fixed at 10 Mbps with 5 ms delay
- packet-size sweep [128, 256, 512, 1024, 1500] B
"""
...
def _configure_wifi(ap_node, sta_node):
    # 802.11g operates in DCF mode, so MAC contention/backoff influences last-hop delay.
...
def run_one(packet_size_bytes, link_type, run_id):
    ...
    # Router1 access-side queue is explicit DropTail with fixed packet depth.
    # Smaller buffers cause earlier drops but lower queueing delay; larger ones postpone drops and raise delay.
    ...
    # Router1/Router2 bottleneck queue depth controls when loss begins relative to saturation.
    bottleneck_devices = bottleneck.Install(routers.Get(0), routers.Get(1))

    if link_type == "wireless":
        last_hop_devices = _configure_wifi(routers.Get(1), receiver.Get(0))
    else:
        wired_last_hop = ns.PointToPointHelper()
        wired_last_hop.SetDeviceAttribute("DataRate", ns.StringValue("10Mbps"))
        wired_last_hop.SetChannelAttribute("Delay", ns.TimeValue(ns.MilliSeconds(2)))
        wired_last_hop.SetQueue("ns3::DropTailQueue<Packet>", "MaxSize", ns.StringValue(ROUTER_QUEUE_MAX_SIZE))
        last_hop_devices = wired_last_hop.Install(routers.Get(1), receiver.Get(0))
    ...
    csma_ip.SetBase(ns.Ipv4Address("10.40.1.0"), ns.Ipv4Mask("255.255.255.0"))
    csma_ip.Assign(csma_devices)

    bottleneck_ip = ns.Ipv4AddressHelper()
    bottleneck_ip.SetBase(ns.Ipv4Address("10.40.2.0"), ns.Ipv4Mask("255.255.255.0"))
    bottleneck_ip.Assign(bottleneck_devices)

    last_hop_ip = ns.Ipv4AddressHelper()
    last_hop_ip.SetBase(ns.Ipv4Address("10.40.3.0"), ns.Ipv4Mask("255.255.255.0"))
    last_hop_interfaces = last_hop_ip.Assign(last_hop_devices)

    if link_type == "wireless":
        receiver_ip = last_hop_interfaces.GetAddress(1)
    else:
        receiver_ip = last_hop_interfaces.GetAddress(1)
...
def main():
    ...
    for packet_size_bytes in PACKET_SIZE_SWEEP_BYTES:
        wireless_metrics = run_one(packet_size_bytes, "wireless", run_id)
        rows.append(
            {
                "link_type": "wireless",
        ...
        wired_metrics = run_one(packet_size_bytes, "wired", run_id)
        rows.append(
            {
                "link_type": "wired",
                "packet_size_bytes": packet_size_bytes,
    ...
    print("\nExperiment 4 Summary (Wired vs Wireless)")
    print("link_type | packet_size | throughput_mbps | delay_ms | loss_pct")
    for row in rows:
        print(
            f"{row['link_type']:>9} | "
